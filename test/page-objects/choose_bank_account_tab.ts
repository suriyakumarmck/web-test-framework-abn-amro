...
class ChooseBankAccountTab extends Page {
    ...
    async inputHouseNoAndZipCode(testid, houseNumber, postalCode) {

        await this.houseNoInputBox.waitForExist()
        await this.houseNoInputBox.scrollIntoView()
        await this.typeInto(await this.houseNoInputBox, houseNumber)
        reporter.addStep(testid, "info", `House number entered sucessfully ${houseNumber}`)

        await this.typeInto(await this.postalCodeInputBox, postalCode)
        reporter.addStep(testid, "info", `Postal code entered sucessfully ${postalCode}`)
    }
    ...
    async validateAutoFill(testid, streetName, town) {

        
        await this.streetNameInputBox.scrollIntoView()
        await this.streetNameInputBox.click()

        let autoGeneratedStreet = await this.streetNameStaticInputBox.getText()
        reporter.addStep(testid, "info", `Street Name is auto detected expected: ${streetName} & actual: ${autoGeneratedStreet}`)
        await chai.expect(autoGeneratedStreet.toLowerCase()).to.equal(streetName.toLowerCase())

        await this.townNameStaticInputBox.scrollIntoView()
        let autoGeneratedTown = await this.townNameStaticInputBox.getText()
        reporter.addStep(testid, "info", `Town Name is auto detected expected : ${town} & actual: ${autoGeneratedTown}`)
        await chai.expect(autoGeneratedTown.toLowerCase()).to.equal(town.toLowerCase())
